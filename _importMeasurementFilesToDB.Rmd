...
This code uploads the ~12 measurement files to DB tables which I hope is more efficient than doing it manually
...
importToServerFolder = "Z:\Population Health Management\Activity Data\processing_folder\Raw_Data\import_measurements/"
measurement_files = c('albumin.csv', 'alcohol_cscore.csv','Blood pressure.csv','BMI.csv',
  'cholesterol.csv','creatinine.csv','efi_category.csv','egfr.csv','FEV1.csv','hba1c.csv',
  'MRC_Dyspnoea.csv')
...
ans = readline("Would you like to clear the measurement database tables in Population_Segmentation? (EXEC sp_XXXX).  Close VPN connection now if it is open.  Enter Y or N.")
...
pathToPackageX = paste0('"', pathToPackage, gsub(".csv", ".dtsx", "covid_antibody_text.csv"), '"')
...
dataFiles = dir(importToServerFolder, "*.csv", ignore.case = FALSE, all.files = FALSE)
...
#for (j in 1:1) {
  ...
  dataFiles = dir(indGroupFolder, "*.csv", ignore.case = FALSE, all.files = FALSE)
  ...
  listOfAttributes = list.files(paste0(attributes_path,"\\", folder_names[j]), pattern="*.csv")
